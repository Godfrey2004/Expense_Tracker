...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 2. PIN SECURITY LOCK SYSTEM
    // ==========================================
    const pinLockOverlay = document.getElementById('pin-lock-overlay');
    const pinLockMsg = document.getElementById('pin-lock-msg');
    ...
    let currentPinInput = '';
    const activePin = localStorage.getItem('ef_pin');

    // Startup lock check
    if (activePin && pinLockOverlay) {
    ...
    function updatePinDots() {
        pinDots.forEach((dot, idx) => {
            if (idx < currentPinInput.length) {
                ...
                dot.style.boxShadow = 'var(--shadow-glow)';
            } else {
    ...
    function handlePinInput(num) {
        ...
        if (currentPinInput.length === 4) {
            // Verify
            if (currentPinInput === localStorage.getItem('ef_pin')) {
                ...
                currentPinInput = '';
                updatePinDots();
                pinLockMsg.textContent = 'Incorrect PIN. Try again.';
                pinLockMsg.style.color = '#FF453A';
                
                // Shake animation trigger
                const container = pinLockOverlay.firstElementChild;
                if (container) {
    ...
    pinKeys.forEach(key => {
        if (key.id !== 'pin-backspace' && key.textContent.trim()) {
            key.addEventListener('click', () => handlePinInput(key.textContent.trim()));
        }
    });
    ...
    function updatePinUI() {
        if (localStorage.getItem('ef_pin')) {
            if (pinStatusText) pinStatusText.textContent = 'Active';
            if (btnTogglePin) btnTogglePin.textContent = 'Disable';
        } else {
            if (pinStatusText) pinStatusText.textContent = 'Inactive';
            if (btnTogglePin) btnTogglePin.textContent = 'Setup';
        }
    }
    updatePinUI();
    ...
    if (btnTogglePin) {
        btnTogglePin.addEventListener('click', async () => {
            const hasPin = localStorage.getItem('ef_pin');
            if (hasPin) {
                const confirmClear = await window.showConfirm("Are you sure you want to disable PIN lock security?", "Disable Security");
                if (confirmClear) {
                    localStorage.removeItem('ef_pin');
                    updatePinUI();
                    window.showToast?.("PIN Lock disabled", "lock_open");
                }
            } else {
                const newPin = await window.showPrompt("Enter a new 4-digit PIN code:", "App PIN Setup");
                if (newPin) {
                    if (/^\d{4}$/.test(newPin)) {
                        localStorage.setItem('ef_pin', newPin);
                        updatePinUI();
                        window.showToast?.("PIN Lock configured successfully", "security");
                    } else {
                        await window.showAlert("Invalid PIN. It must be exactly 4 numeric digits.", "Setup Error");
                    }
                }
            }
        });
        ...
        list.forEach(goal => {
            ...
            card.innerHTML = `
                ...
                <div style="display: flex; justify-content: space-between; font-size: 13px; color: var(--text-secondary);">
                    ...
                    <span>Date: ${new Date(goal.date).toLocaleDateString()}</span>
                </div>
        ...
        goals.forEach(goal => {
            ...
            if (rem > 0) {
                ...
                if (daysLeft > 0) {
                    ...
                    aiInsights.push({
                        emoji: '🎯',
                        text: `To achieve <strong>"${goal.name}"</strong> by ${new Date(goal.date).toLocaleDateString()}, you need to save an average of <strong>${window.formatCurrency(requiredDaily)}/day</strong>.`
                    });
                }
            }
        });
